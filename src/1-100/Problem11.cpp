...
typename std::conditional<(sizeof(A) <= sizeof(B)), B, A>::type max(
    const A& a, const B& b) {
    if (a < b) return b;
    return a;
}
...
void p11(std::ostream& os) {
    std::uint32_t maxProduct = 0;
    for (int a{0}; a < 20 - 3; ++a) {
        for (int b{0}; b < 20 - 3; ++b) {
            maxProduct = max(maxProduct, ((grid[a][b]) * (grid[a][b + 1]) *
                                          (grid[a][b + 2]) * (grid[a][b + 3])));
            maxProduct = max(maxProduct, (grid[a][b] * grid[a + 1][b] *
                                          grid[a + 2][b] * grid[a + 3][b]));
            maxProduct =
                max(maxProduct, (grid[a][b] * grid[a + 1][b + 1] *
    ...
    for (int x{3}; x < 20; ++x) {
        for (int y{0}; y < 20 - 3; ++y) {
            maxProduct =
                max(maxProduct, (grid[y][x] * grid[y + 1][x - 1] *
