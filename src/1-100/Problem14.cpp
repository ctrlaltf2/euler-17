...
int hailstoneBouncesOf(int /* num */,
                       std::unordered_map<int, int>& /* previousBounces */);

void p14(std::ostream& os) {
    std::unordered_map<int, int> memoizedBounces;
    memoizedBounces.reserve(3'721'303);  // Reserve just the right amount of
                                         // buckets to prevent rehashing. Magic
                                         // number comes from the number of
                                         // buckets in the map after this is
                                         // run. This shaves off ~0.95 seconds
                                         // on my 4 core 3.1GHz
...
int hailstoneBouncesOf(const int num,
                       std::unordered_map<int, int>& previousBounces) {
    ...
    for (const auto& i : cache) previousBounces[i] = bounces--;
